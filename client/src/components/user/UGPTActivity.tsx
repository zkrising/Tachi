...
// Records activity for a group of users on a GPT. Also used for single users, sometimes.P
export default function UGPTActivity({
	...
	duration: "month" | "3month" | "year";
	setDuration: SetState<"month" | "3month" | "year">;
}) {
	const rivalMap = CreateUserMap(users);

	return (
		<>
			<Select value={duration} setValue={setDuration}>
			...
			<Col xs={12} className="text-center">
				<div className="timeline timeline-2">
					...
					{data.map((e) => {
						const rival = rivalMap.get(
							e.type === "SCORES" ? e.scores[0]?.userID : e.userID
						);
						...
						return e.type === "SCORES" ? (
							<ScoresActivity data={e} rival={rival} />
						) : (
							<SessionActivity data={e} rival={rival} />
						);
					})}
					<div className="timeline-item">
						<div className="timeline-item">
							...
							>
								This is the end of the recent activity!
							</div>
						</div>
					</div>
				</div>
...
function ScoresActivity({ data, rival }: { data: ClumpedActivityScores; rival: UserDocument }) {
				...
				>
					<div className="mr-3" style={{ width: "70%", textAlign: "left" }}>
						...
						<span style={{ fontSize: "1.15rem" }} className="ml-2">
							{rival.username} highlighted {subMessage}!
						</span>
					</div>
			...
			>
				...
				>
					<div className="mr-3" style={{ width: "70%", textAlign: "left" }}>
						...
						/>
						<span className="ml-2">
							{rival.username} had a session '{data.name}' with{" "}
							{data.scoreInfo.length}{" "}
							{data.scoreInfo.length === 1 ? "score" : "scores"}.
						</span>
						<br />
						{data.desc && <span className="text-muted">{data.desc}</span>}
					</div>
