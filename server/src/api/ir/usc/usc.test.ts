...
t.test("GET /api/ir/usc", async (t) => {
    ...
    const res = await mockApi.get("/api/ir/usc").set("Authorization", "Bearer foo");
...
t.test("GET /api/ir/usc/charts/:chartHash", (t) => {
    ...
    t.test("Should return 42 if the chartHash doesn't match a chart.", async (t) => {
        const res = await mockApi
            .get("/api/ir/usc/charts/INVALID_HASH")
            .set("Authorization", "Bearer foo");
...
t.test("GET /api/ir/usc/:chartHash/record", (t) => {
    t.beforeEach(ResetDBState);
    t.beforeEach(InsertFakeUSCAuth);
    TestAuth("/api/ir/usc/:chartHash/record");

    t.test("Should return 42 if the chartHash doesn't match a chart.", async (t) => {
        const res = await mockApi
            .get("/api/ir/usc/charts/INVALID_HASH/record")
            .set("Authorization", "Bearer foo");
    ...
    t.test("Should return 44 if there are no scores on the chart.", async (t) => {
        const res = await mockApi
            .get("/api/ir/usc/charts/USC_CHART_HASH/record")
            .set("Authorization", "Bearer foo");
...
t.test("GET /charts/:chartHash/leaderboard", (t) => {
    t.beforeEach(ResetDBState);
    t.beforeEach(InsertFakeUSCAuth);
    TestAuth("/api/ir/usc/:chartHash/leaderboard");

    t.test("Should return 40 if mode is invalid", async (t) => {
        const res = await mockApi
            .get("/api/ir/usc/charts/USC_CHART_HASH/leaderboard")
            .set("Authorization", "Bearer foo");

        t.equal(res.body.statusCode, 40);

        const res2 = await mockApi
            .get("/api/ir/usc/charts/USC_CHART_HASH/leaderboard?mode=invalid")
            .set("Authorization", "Bearer foo");
    ...
    t.test("Should return 40 if N is invalid", async (t) => {
        const res = await mockApi
            .get("/api/ir/usc/charts/USC_CHART_HASH/leaderboard?mode=best")
            .set("Authorization", "Bearer foo");

        t.equal(res.body.statusCode, 40);

        const res2 = await mockApi
            .get("/api/ir/usc/charts/USC_CHART_HASH/leaderboard?mode=best&n=foo")
            .set("Authorization", "Bearer foo");
    ...
    t.test("Should return empty arr for mode = best if no scores", async (t) => {
        const res = await mockApi
            .get("/api/ir/usc/charts/USC_CHART_HASH/leaderboard?mode=best&n=5")
            .set("Authorization", "Bearer foo");
    ...
    t.test("Should return scorePBs for mode = best", async (t) => {
        ...
        const res = await mockApi
            .get("/api/ir/usc/charts/USC_CHART_HASH/leaderboard?mode=best&n=2")
            .set("Authorization", "Bearer foo");
        ...
        const res2 = await mockApi
            .get("/api/ir/usc/charts/USC_CHART_HASH/leaderboard?mode=best&n=1")
            .set("Authorization", "Bearer foo");
