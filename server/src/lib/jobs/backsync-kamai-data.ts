 ...
 */
export async function BacksyncKamaiSongsAndCharts() {
	for (const branch of ["staging", `release/${VERSION_INFO.major}.${VERSION_INFO.minor}`]) {
		...
		for (const game of ["iidx"] as const) {
		...
		await repo.CommitChangesBack(`Backsync Kamai Songs/Charts ${new Date().toISOString()}`);
