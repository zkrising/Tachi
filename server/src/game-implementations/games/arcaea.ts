import { GPTServerImplementation } from "game-implementations/types";
...
export const ARCAEA_IMPL: GPTServerImplementation<"arcaea:Single"> = {
    ...
    classDerivers: {
        badge: (ratings) => {
            ...
            if (potential >= 13.00) {
                return "THREE_STARS";
            } else if (potential >= 12.50) {
                return "TWO_STARS";
            } else if (potential >= 12.00) {
                return "ONE_STARS";
            } else if (potential >= 11.00) {
                return "RED";
            } else if (potential >= 10.00) {
                return "PURPLE";
            } else if (potential >= 7.00) {
                return "ASH_PURPLE";
            } else if (potential >= 3.50) {
                return "GREEN";
            }
    ...
    goalProgressFormatters: {
        ...
        grade: (pb, gradeIndex) => GradeGoalFormatter(ARCAEA_GBOUNDARIES, pb.scoreData.grade, pb.scoreData.score, ARCAEA_GBOUNDARIES[gradeIndex]!.name, (v) => `$v`),
    },
    goalOutOfFormatters: {
