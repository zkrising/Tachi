...
export default function JubeatScoreTable({
	reqUser,
	dataset,
	pageLen,
	userCol = false,
	...
	dataset: ScoreDataset<"jubeat:Single">;
	pageLen?: integer;
	playtype: Playtypes["jubeat"];
	userCol: boolean;
}) {
	const headers: Header<ScoreDataset<"jubeat:Single">[0]>[] = [
	...
	return (
		<TachiTable
			...
			headers={headers}
			entryName="Scores"
			searchFunctions={CreateDefaultScoreSearchParams("jubeat", "Single")}
...
function Row({
	...
	sc: ScoreDataset<"jubeat:Single">[0];
	reqUser: PublicUserDocument;
	userCol: boolean;
}) {
	...
	return (
		...
		>
			...
			<DifficultyCell chart={sc.__related.chart} game="jubeat" />
