...
function fixBrokenData(tier, title, diff, level, packDir) {
	...
	if (title === "Amnèhilesie" && [
		"us6 NOV 6",
		"us12 ADV 12"
	].includes(tierDiffLevel)) {
		return ["Amnéhilesie", diff, level, `\\${packDir}\\`];
	}
	if (title === "White Wail -the \"X-carols\"-" && [
		"us9 NOV 9",
		"us16 ADV 16",
		"os3 EXH 19",
		"os10 INF 20",
	...
	if (title === "#curtaincall(finale)" && tierDiffLevel === "os5 INF 20") {
		return ["#curtaincall", diff, level, "B4UT&KUOC K-shoot mania Package 2016\\03_BOSS_B4UT\\04_#curtaincall_finale\\"];
	}
	if (title === "#curtaincall" && tierDiffLevel === "os9* INF 20") {
		return ["#curtaincall", diff, level, "B4UT&KUOC K-shoot mania Package 2016\\03_BOSS_B4UT\\03_#curtaincall\\"];
	}
	if (title === "双極のバラキエル" && tierDiffLevel === "os6 INF 20") {
		return [title, diff, "19", `\\${packDir}\\`];
	}
	if (title === "Modern Times Paradox" && tierDiffLevel === "us6 NOV 6" ||
...
for (const tableFile of options.charts) {
	...
	for (const chartData of tableData) {
		...
		let dbRowMatches = dbRows.filter(row =>
			row.title === title &&
		...
		if (dbRowMatches.length === 0) {
			console.log(`Couldn't find matching row for ${title} (${difficulty} ${level}) in ${tier}. (From package ${pack})`);
			continue;
		} else if (dbRowMatches.length !== 1) {
			if (dbRowMatches.length === 2 && dbRowMatches.filter(row => row.path.endsWith("data.ksh")).length === 1) {
