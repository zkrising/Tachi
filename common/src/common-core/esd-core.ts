...
function StdDeviationToPercent(
    ...
    for (let i = 0; i < judgements.length; i++) {
        ...
        let nVal = CDFBetween(lastJudgeMSBorder, judge.msBorder, MEAN, stddev ** 2) * judge.value;
...
export function CalculateESD(
    ...
    if (errOnInaccuracy) {
        ...
        console.warn(
            `(ESD-JS) Did not reach value within MAX_ITERATIONS (${MAX_ITERATIONS}), returning last estimate (${estSD}), which was generated from ${percent}.`
        );
        return estSD;
    }
}
